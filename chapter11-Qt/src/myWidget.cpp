...
void MyGlWidget::paintGL()
{
    ...
    glm::mat4 projection = glm::perspective(glm::radians(fov), (float)clientArea.width() / clientArea.height(), 0.1f, 100.0f);
...
{
    // firstMouse = true;
    QPoint glClientCenter = glWidget->geometry().center();
    QPoint glClientGlbCenter = glWidget->mapToGlobal(glClientCenter);
    QCursor::setPos(glClientGlbCenter);
    
    QPoint glClientParentCenter = glWidget->mapToParent(glClientCenter);
    lastX = glClientParentCenter.x();
    lastY = glClientParentCenter.y();
    qDebug() << "enter" << lastX << "," << lastY;
}
...
void MyWidget::mouseMoveEvent(QMouseEvent *event)
{
    ...
    double yoffset = lastY - event->y(); // window Y axis reverse of GL coordinate
...
{
    int yoffset = event->angleDelta().y() / 40;
